...
### 57. Malicious LLM-Based Conversational AI Makes Users Reveal Personal Information
...
**🎯 目标问题**
本文研究的是一类新型威胁：基于LLM的对话式AI（CAI）被恶意设计，利用系统提示词主动诱导用户披露个人信息。威胁模型针对的是在OpenAI GPT Store、FlowGPT等平台上由第三方部署的定制化CAI，攻击者无需技术背景，仅凭精心设计的system prompt即可构建此类系统。由于用户普遍将CAI视为中立或有益的工具，其自我保护意识较弱，且此类攻击可自动化、规模化地针对海量用户，危害尤为严重。此前研究仅关注用户在与良性CAI交互时被动泄露信息的问题，而对主动式、有策略性地诱导披露这一场景完全空白。

**🔧 核心技术**
研究采用随机对照实验（RCT）框架，502名参与者被随机分配与12种CAI之一实时交互（4种prompt策略×3种LLM的全因子设计）。系统提示词设计分两层：第一层利用角色扮演（"侦探""调查员"）绕过LLM内置安全护栏；第二层实现三种恶意信息诱导策略——Direct（直接询问个人信息）、User-benefit（先提供价值再索取信息，利用隐私-效用权衡心理）、Reciprocal（通过共情、情感支持和互惠叙事将信息收集伪装成社交互动，利用隐私的社会性本质）。个人信息抽取采用NuExtract（基于LLM的结构化信息提取工具）自动识别103类个人信息类别，Cohen's kappa=0.818验证了其可靠性。统计分析采用Kruskal-Wallis非参数检验配合Dunn事后检验（BH校正），并通过归纳/演绎双路径定性主题分析补充解释量化结果。

**📝 评价**
本文在研究设计上有显著亮点：502人的RCT规模、预注册的统计功效分析（medium effect size，power=0.95）、以及量化+质化的混合方法，构成了目前该方向最严谨的实证基础。核心发现颇具冲击力：R-CAI（互惠策略）在诱导用户披露的个人信息量与B-CAI相比有显著差异（p<0.001），且被参与者感知为与良性CAI同等可信、同等低风险，而实际上提供假数据的比例仅10%（D/U策略达40-42.5%），说明互惠策略在不触发防御心理的情况下获得了更真实的数据，这是最关键的安全发现。然而存在几点局限：第一，所用LLM均为开源模型（Llama、Mistral），未直接在GPT-4或Claude上验证，而文中仅以相似度评分间接类比商业模型，说服力有限；第二，参与者样本高度集中于英语母语国家（英国33%+美国23%+欧洲25%），文化背景对隐私披露行为的影响未被充分检验；第三，"个人信息量"以NuExtract检测到的数据点总数计算，未区分信息敏感程度，中位数差异的实际危害可能被平均化掩盖；第四，Incomplete Disclosure Protocol的使用虽符合伦理规范，但也可能造成参与者与真实世界中毫无知情的普通用户之间的行为差异，限制了生态效度的泛化程度。

---

### 58. EmbedX: Embedding-Based Cross-Trigger Backdoor Attack Against Large Language Models
...
### 59. Exploiting Task-Level Vulnerabilities: An Automatic Jailbreak Attack and Defense Benchmarking for LLMs
...
### 60. Effective PII Extraction from LLMs through Augmented Few-Shot Learning
...
### 61. PrivacyXray: Detecting Privacy Breaches in LLMs through Semantic Consistency and Probability Certainty
...
### 63. TwinBreak: Jailbreaking LLM Security Alignments based on Twin Prompts
...
### 64. Unsafe LLM-Based Search: Quantitative Analysis and Mitigation of Safety Risks in AI Web Search
...
### 65. Prompt Obfuscation for Large Language Models
...
### 66. Exposing the Guardrails: Reverse-Engineering and Jailbreaking Safety Filters in DALL·E Text-to-Image Pipelines
...
### 67. Game of Arrows: On the (In-)Security of Weight Obfuscation for On-Device TEE-Shielded LLM Partition Algorithms
...
### 68. SelfDefend: LLMs Can Defend Themselves against Jailbreaking in a Practical Manner
...
### 69. Mirage in the Eyes: Hallucination Attack on Multi-modal Large Language Models with Only Attention Sink
...
**🎯 目标问题**
本文针对多模态大语言模型（MLLM）的幻觉问题设计主动攻击，目标是通过对抗性视觉输入放大模型生成与图像内容不符的对象、属性和关系描述。威胁模型设定攻击者具备对某代理MLLM的白盒访问权限，可计算梯度并构造对抗样本，然后将其迁移攻击黑盒目标模型（包括GPT-4o、Gemini 1.5等商业API）。该问题的重要性在于：随着MLLM被广泛部署于医疗推理、自动驾驶、客服系统等高风险场景，攻击者可通过用户可上传图片的接口注入对抗样本，导致下游系统基于虚假描述做出错误决策，且因响应质量未明显下降而难以被检测。

**🔧 核心技术**
核心思路是将MLLM幻觉的内因（注意力汇聚/attention sink）转化为可利用的攻击面。方法分三步：(1) 机制分析——通过CLIPScore统计指令微调数据集（LLaVA-Instruct-150k等）中图文相关性随句子位置下降的现象，并实验验证注意力汇聚token恰好出现在图文相关性的拐点处，且其中间层嵌入与全局多模态输入嵌入的余弦相似度显著高于其他token，说明汇聚token聚合了误导性全局信息。(2) 潜在汇聚位置识别——在每轮对抗优化中，计算已生成token的中间层隐藏状态与全局输入均值向量的余弦相似度，选相似度最高者作为目标汇聚位置（式6-7），动态适应当前响应结构。(3) 双目标对抗优化——设计注意力损失（对目标位置后续token的注意力分布施加交叉熵，强制形成柱状注意力模式，式8）和嵌入损失（hinge loss驱使目标token嵌入向全局输入嵌入靠近但不超过阈值σ，使其注入不完整的误导性全局信息，式9），通过PGD式迭代优化视觉扰动δ（ε约束在2/255至8/255范围内）。整个过程无需预定义目标响应文本，对通用视觉-文本输入均可适用。

**📝 评价**
创新性方面，论文首次将attention sink的被动观测转化为主动攻击原语，设计动机清晰且有理论分析支撑，区别于依赖预设目标文本的传统对抗攻击，是攻击目标和方法论层面的真正创新。亮点在于攻击的无目标性（不依赖predefined responses）和黑盒迁移性，Tab.3中LLaVA-1.5→Shikra的HSR提升达10.90%，迁移效果在同架构族内较好。然而实验存在若干不足：第一，干净输入的基线幻觉率本身已极高（InstructBLIP HSR为62.91%，LLaVA-1.5高达71.68%），说明这些模型本就严重幻觉化，攻击带来的增量（多数3-7%）在实际威胁意义上是否足够显著值得商榷；第二，在强迁移场景（如InstructBLIP→LLaVA-1.5）出现HWPI负增长（-8.57），说明跨架构迁移不稳定，论文对此解释不足；第三，对GPT-4o和Gemini 1.5商业API的攻击效果数据在本文截断处未完整呈现，无法评估其实际影响；第四，对已有缓解策略（OPERA等三类）的绕过实验虽声称有效，但具体数据未在正文中完整展示。整体而言论文思路新颖、工程实现完整，但攻击增量的实际危害性论证略显薄弱。

---

### 70. Make Agent Defeat Agent: Automatic Detection of Taint-Style Vulnerabilities in LLM-based Agents
...
### 71. Cloak, Honey, Trap: Proactive Defenses Against LLM Agents
...
### 72. GPTracker: A Large-Scale Measurement of Misused GPTs
...
**🎯 目标问题**
本文关注OpenAI GPT Store中被滥用的GPT智能体（misused GPTs）这一安全问题。威胁模型是：恶意构建者利用OpenAI平台宽松的定制化机制，创建用于非法活动、欺诈、传播恶意软件、仇恨言论、色情内容等十类违禁场景的GPT智能体，并借助外部API、URL重定向、隐藏意图等手段绕过平台审核。该问题的重要性在于GPT Store已聚集数百万GPT应用，覆盖全球用户，而现有研究缺乏对滥用GPT生态系统的系统性大规模测量，平台审核机制的实际效果几乎未被量化评估。

**🔧 核心技术**
论文构建了GPTRacker框架，分两个模块：一是元数据采集器，利用10000个高频英语词作为搜索关键词，通过Playwright驱动的网络爬虫每两周一轮从官方GPT Store采集GPT元数据，历时8个月完成16轮采集；二是对话提取器，开发Chrome扩展拦截WebSocket，自动化交互并采集含系统初始化、函数调用等用户不可见内层流程（flow）的完整对话流。识别滥用GPT的方法采用半自动化管道：以GPT-4o-mini作为打分器，对每个GPT与每个违禁场景评估风险分（每次询问3次取均值），在人工标注的459个样本上选择最优阈值0.70（F1最高），再由两名作者对3166个候选GPT进行人工复核（一致率85.66%），最终确认2051个滥用GPT。动态分析通过自动触发对话起始提示采集28464条flow，归纳出四种对话模式（无工具、内置工具、外部API、混合），并用VirusTotal对所有出现的域名进行安全扫描。

**📝 评价**
本文是GPT Store滥用问题的首项大规模系统性测量研究，数据规模（755,297个GPT、16轮纵向追踪）和方法严谨性在同类工作中较为突出，负责任披露并推动OpenAI下架1316个GPT这一实际影响也值得肯定。核心创新在于将静态元数据分析与基于WebSocket拦截的动态flow分析结合，揭示了外部API使得滥用GPT回答率平均提升22.81%这一具体且可验证的发现。批判性来看，识别方法仅能发现在名称、描述或对话提示中"显式暴露"意图的GPT，系统提示（system prompt）因权限限制无法获取，这意味着那些通过隐晦指令包装的高级对抗性GPT会被大量漏过，论文自身也承认这一局限但未能量化漏报率。阈值选取（0.70）虽经过人工标注验证，但459个标注样本偏少，且两名作者仅81.70%的初始一致率表明边界样本判定存在主观性。此外，10000个英语词汇的搜索覆盖策略对纯非英语GPT的覆盖率未作系统评估，而CSAM类别零发现究竟是有效审查还是检测缺位，论文未作深入讨论。外部API的恶意内容分析因配置复杂被留为未来工作，是一个明显未填补的漏洞。

---

### 73. Fun-tuning: Characterizing the Vulnerability of Proprietary LLMs to Optimization-Based Prompt Injection Attacks via the Fine-Tuning Interface
...
### 74. Alleviating the Fear of Losing Alignment in LLM Fine-tuning
...
### 75. Fuzz-Testing Meets LLM-Based Agents: An Automated and Efficient Framework for Jailbreaking Text-to-Image Generation Models
...
**🎯 目标问题**
文本到图像（T2I）生成模型（如Stable Diffusion、DALL·E 3）内置的安全过滤器可被精心构造的提示词绕过，从而生成NSFW内容（裸露、暴力等）。威胁场景为纯黑盒攻击：攻击者仅能通过API查询目标模型，无法获取模型内部参数或安全过滤器实现细节。现有攻击方法存在四类缺陷：白盒依赖不实用、生成的提示词不自然易被困惑度检测、搜索空间受固定模板限制、以及查询次数过多导致高昂经济成本。本文旨在同时克服上述缺陷，实现高效、自然、语义保真的黑盒越狱攻击。

**🔧 核心技术**
JailFuzzer将模糊测试（fuzzing）范式与LLM智能体结合：框架由种子池（seed pool）、引导变异引擎（mutation agent）和预言函数（oracle agent）三部分构成。变异智能体以VLM（LLaVA-1.5-13b或ShareGPT4V-13b）为大脑，通过多模态感知判断当前提示词是否绕过过滤器，并利用基于ICL的内存模块从历史成功样本中提取共性策略（Identify+Guide两阶段反思），生成新候选提示词。预言智能体以LLM（Vicuna-1.5-13b）为大脑，通过角色切换分别模拟安全过滤器评估绕过得分、或评估语义相似度，提前筛选高潜力候选词以减少对目标模型的真实查询次数。语义保真度通过CLIP-ViT-Base-Patch32计算图像与原始敏感提示词之间的CLIPScore（阈值δ=0.26）来量化。整体采用受指数退避策略启发的多轮攻击流（multi-loop attack flow），对失败提示词积累经验后在后续轮次重新尝试，避免陷入局部最优。

**📝 评价**
创新性上，将经典软件安全测试中的fuzzing范式移植到LLM驱动的提示词攻击场景具有方法论意义，ICL-based记忆模块与两阶段Reflection机制也是有价值的设计细节。实验较为充分，覆盖4个T2I模型、6类安全过滤器、200个NSFW提示词，并与SneakyPrompt、Ring-A-Bell、DACA进行了对比，结果显示绝大多数过滤器接近100%绕过率且平均仅需4.6次查询，有说服力。然而存在几个值得批评的局限：第一，re-use旁路率在image-based过滤器上大幅下滑（约50-67%），说明生成的越狱提示词对特定生成随机性依赖较强，实际复用价值有限，但论文对此分析不足。第二，DALL·E 3上的FID分数极高（~294，远高于SD模型），表明语义偏移严重，仅用"81.93%绕过率"掩盖了图像质量与语义忠实度的实质退化。第三，攻击者自身需要部署本地VLM（LLaVA/Vicuna）才能运行系统，这一计算成本与复杂度在威胁模型中被轻描淡写，实际门槛并不低。第四，论文对防御侧的讨论流于表面，未讨论针对JailFuzzer本身的检测或对抗措施，作为安全研究其双面性分析不完整。

---

### 76. BAIT: Large Language Model Backdoor Scanning by Inverting Attack Target

- **会议/年份**: SP 2025
- **作者**: Guangyu Shen, Siyuan Cheng 0005, Zhuo Zhang 0002, Guanhong Tao 0001, Kaiyuan Zhang 0002, Hanxi Guo 等
- **类别**: defense
- **分析依据**: 📄 全文
- **是否开源**: ✅ 是 — https://github.com/SolidShen/BAIT
- **链接**: [官方链接](https://doi.org/10.1109/SP61157.2025.00103) · [代码](https://github.com/SolidShen/BAIT) · [DBLP](https://dblp.org/rec/conf/sp/Shen0000GY0AM025)
- **置信度**: 0.93

**🎯 目标问题**
本文解决大型语言模型（LLM）的后门（木马）检测问题。攻击者通过数据投毒在训练阶段向LLM注入触发词，使模型在检测到触发词时输出预设的恶意目标序列（如政治煽动内容、数据库删除指令），正常输入下则行为正常。现有针对判别式模型的触发词逆向（trigger inversion）方法面临LLM输出空间指数级爆炸问题（词汇量32000、响应长度10时搜索空间约达32000^10），在SOTA LLM后门攻击上仅能达到0.61 ROC-AUC，实际无效。该威胁因LLM训练过程对用户不透明、部署范围广泛而尤为紧迫。

**🔧 核心技术**
BAIT的核心洞见是放弃逆向触发词，转而逆向攻击目标序列（target inversion）。理论基础来自对CLM自回归训练范式的分析：作者在合理假设下证明（Theorem 4.4），即使不提供触发词，后门目标序列中各token间也存在强因果关系——给定前缀target tokens，模型预测下一个target token的期望概率存在可证明的正下界，且该下界大于其他任意token的概率。基于此，BAIT通过枚举词汇表中每个token作为候选初始token，利用Property 5.1递归贪心构建候选序列，并用Q-SCORE（各步骤期望概率均值）来量化序列的"后门可疑度"。为解决有限样本（默认20条）带来的期望估计不确定性，引入自熵（self-entropy）作为动态指标：低熵时执行贪心选择，中等熵时切换为Top-K前向探查（选前K候选token并各做一步前瞻），高熵时提前终止当前序列搜索，从而在鲁棒性与效率间取得平衡。整个过程只需软标签黑盒访问（模型输出的token概率分布），无需白盒梯度或任何关于触发词/目标词的先验知识。

**📝 评价**
创新性较强：将检测目标从触发词逆向转为目标序列逆向，是一次有理论支撑的方向性突破，理论推导在合理假设下完整，并用4种架构的真实后门LLM做了假设验证（图4、图5），说服力较充分。实验规模可信：153个LLM、8种架构、6种攻击类型，与5个基线对比，平均ROC-AUC达0.98（基线均值0.61），在TrojAI竞赛Round 19中取得满分1.00，外部验证增强了可信度。批判性不足之处：(1) 方法强依赖"universal target"假设，即触发词存在时模型对不同输入始终输出相同目标序列，对目标序列随提示语义变化的paraphrased攻击仅在附录中简要涉及，覆盖不完整；(2) 需要访问完整token概率分布（软标签黑盒），实际大量商业API仅输出文本或top-token，部署场景受限；(3) 词汇表全量枚举在Gemma-7B（词汇量256000）上K=5时需超过4000秒，计算开销问题作者虽有动态熵策略缓解，但在大词汇表模型上的工程可行性仍是瓶颈；(4) Theorem 4.4的理论下界在后门效果较弱场景（如图5b，LLaMA3-8B，α=0.8）时与实测值偏差明显，作者承认假设在实践中可能不严格成立，这对低毒化率或防御性微调后的模型检测能力构成潜在隐患。

---

### 77. Prompt Inversion Attack Against Collaborative Inference of Large Language Models
...
### 78. Make a Feint to the East While Attacking in the West: Blinding LLM-Based Code Auditors with Flashboom Attacks
...
**🎯 目标问题**
LLM-based代码审计工具（如GitHub Copilot）被广泛用于漏洞检测，但其核心注意力机制存在可被利用的弱点。威胁模型为：攻击者将含有真实漏洞（漏洞I）的代码上传至GitHub/Stack Overflow，开发者下载后使用LLM审计工具检查，攻击目标是使真实漏洞绕过LLM检测。该场景高度现实：GitHub Copilot已有130万付费用户、5万+组织接入，供应链级别的漏洞注入攻击若能同时规避LLM审计则危害极大。不同于针对提示词或模型参数的攻击，本文攻击者只能控制代码内容本身。

**🔧 核心技术**
核心思路是利用注意力机制的"有限注意力资源"特性：在目标漏洞代码旁插入能吸引更高注意力权重的代码片段（Flashboom），从而"稀释"LLM分配给真实漏洞的注意力，导致误判为安全。初步实验证明，插入"漏洞代码+该漏洞的补丁"组合（即一个虚假的漏洞-修复对）最有效：LLM被训练成关注漏洞模式，故此类代码天然吸引高注意力，而补丁的存在让LLM误认为已处理安全问题。为实现规模化，开发了自动化工具Crazy-Ivan，分三个阶段运作：Phase I从大型代码库（Messiq智能合约/LeetCode题解）计算函数级注意力得分（将(T,T,L,H)四维张量聚合为每函数一个标量），选出最高注意力函数Fmax；Phase II用GPT-4o自动补全Fmax的依赖变量和辅助函数，使其可独立编译并重命名以避免命名冲突；Phase III将候选Flashboom插入目标漏洞代码，验证真实漏洞函数的注意力权重是否显著下降，以此确认致盲效果。整个流程无需手动验证每个样本。

**📝 评价**
创新性较强：将LLM注意力机制作为独立攻击面进行系统建模，与梯度对抗样本或提示注入攻击路径不同，具有一定原创价值。实验设计值得肯定：在6个模型上测试（参数量从2.6B到>1T），引入CoT+ICL使LLM处于最有利检测状态（即攻击者面临最强防御）仍能成功，且在GitHub Copilot上完成端到端PoC验证。然而存在明显局限：第一，BSR在小模型（CodeLlama 7B达96.3%、Gemma 2.6B达83.05%）上效果突出，但论文未明确报告GPT-4o的BSR数字，这一关键数据缺失令人怀疑商用最强模型上的实际效果；第二，攻击假设"开发者不使用非LLM静态扫描工具"过于理想化，真实流水线通常会叠加SAST工具（如Semgrep、CodeQL）；第三，语义相似度0.92看起来较高，但Flashboom实际上是插入整块新函数代码，有经验的人工审查者很容易察觉代码库中突然出现的不相关函数，隐蔽性分析不够严谨；第四，该攻击依赖对目标LLM注意力机制的白盒访问来生成Flashboom，而在实际场景中攻击者针对闭源商业模型（如Copilot）时只能依赖迁移性，迁移成功率数据（Gemma >87%）仅在开源模型之间测试，对真实商用模型的迁移效果未充分验证。

---

### 79. DataSentinel: A Game-Theoretic Detection of Prompt Injection Attacks
...
### 80. Modifier Unlocked: Jailbreaking Text-to-Image Models Through Prompts
...
**🎯 目标问题**
本文针对文生图模型（DALL·E 3、Midjourney、Imagen 3、Stable Diffusion 3等）的安全过滤机制展开越狱攻击研究。威胁模型设定为黑盒对手，仅能通过提示词接口与模型交互，无需了解模型内部结构或过滤器实现细节。现有防御体系由预过滤器（文本层面检测敏感词）和后过滤器（图像层面检测NSFW内容）构成，但后过滤器对特定艺术风格存在盲区——即使这些风格携带明显的NSFW倾向，模型仍会正常输出。现有攻击方法集中于替换提示词中的"主体"（subject），已被持续迭代的过滤器逐步拦截；而"修饰符"（modifier，即艺术风格、媒介等描述性元素）作为攻击向量的潜力长期被忽视，构成该论文的核心切入点。

**🔧 核心技术**
MODX围绕"修饰符"构建越狱框架，分两大阶段运作。第一阶段构建恶意修饰符数据集（MMD）：通过CLIP Interrogator分析从Reddit/4chan采集的600张NSFW图像、抓取DeviantArt/Saatchi Art/PromptHero上的NSFW相关提示词，汇总得到涵盖artist、medium、movement、flavor、trending五类共717个修饰符。第二阶段构造越狱提示词，分两个约束条件串行筛选：约束#1利用CLIP计算修饰符与目标NSFW类别描述（GPT-4o生成同义词扩展）的嵌入相似度，从每类中选取top-p修饰符，组合产生p^5个候选提示词；约束#2用SBERT计算候选越狱提示词与原始提示词的语义相似度，选取top-k作为最终攻击提示词。在场景二（含敏感主体）中，额外使用GPT-4o对敏感词进行语义替换以绕过预过滤器。理论层面，作者借助Lipschitz连续性（对文本编码器和噪声预测函数分别建立上界）和Wasserstein距离，证明了修饰符对模型输出的影响有界、同时可使生成图像分布向NSFW靠拢，并给出两个约束同时满足时修饰符范数的存在性证明。

**📝 评价**
本文的创新点是实质性的：将攻击目标从"主体"转向"修饰符"，是对现有越狱攻击范式的有效补充，揭示了过滤机制中艺术风格类提示词的真实盲区，具有较强的攻击实践意义。实验覆盖四个主流商业模型，在场景一下平均BPR达0.92、ASR达0.74，场景二下分别为0.79和0.53，并与三类baseline（HM Attack、SurrogatePrompt、SneakyPrompt）进行对比，结果总体领先。然而存在若干值得批评的局限：第一，理论推导中的三个假设（编码器Lipschitz连续性、噪声预测函数Lipschitz连续性、良性与NSFW分布Wasserstein距离恒为正常数∆）对闭源商业模型并无经验验证，理论分析更像是事后合理化的形式框架，而非驱动设计决策的工具；第二，论文明确指出未使用人工评估，完全依赖多头安全分类器（Q16等）衡量NSFW程度，而该分类器与目标模型内置过滤器并非同一系统，存在评估口径不一致的潜在偏差；第三，MMD数据集以"受限访问"方式发布，可复现性存在障碍；第四，论文对防御侧的讨论相当薄弱，未深入分析针对修饰符攻击的可行防御方案（如将修饰符纳入预过滤检测），使得安全意义的完整性不足。
...
**🎯 目标问题**
LLM应用商店（如GPT Store、FlowGPT、Poe、Coze等）的大规模安全风险尚无系统性研究。论文将威胁模型分为三层：（1）滥用潜力层——应用描述与实际指令不一致、敏感数据过度收集、恶意开发者域名；（2）恶意意图层——应用内嵌仇恨言论、自我伤害、极端主义等有害内容；（3）可利用漏洞层——应用可被攻击者利用生成恶意软件、发动钓鱼攻击等。低门槛创建机制加之监管执行不力，使得百万量级的自定义LLM应用成为潜在攻击面，危害范围远超传统恶意软件生态。

**🔧 核心技术**
论文提出三层关切框架并配套自动化检测流水线。静态分析方面，自建ToxicDict——覆盖14类有毒词汇、8种语言、共31,783条词条，来源包括平台政策、公开数据集、GPT-4o扩展与翻译；基于ToxicDict做规则化字符串匹配与正则扫描，并设置动态"过滤词"列表迭代降噪。动态分析方面，以Llama3-8B为基础构建自精炼毒性检测器，对14类有害类别打分（0-1），检测失败时随机抽取10条挑战样例人工标注后更新模型；另以Llama3-8B开发描述-指令一致性分析工具，输出0-1的一致性评分。恶意行为验证设计五类攻击任务（恶意软件生成、钓鱼、数据窃取、DoS、虚假信息），以CRR/FC/CC/CA/MEE五项指标衡量，全部在虚拟机隔离环境中执行，使用常规prompt而非越狱手法以体现内生漏洞。数据采集通过Selenium爬取六大平台共786,036个应用，并借助VirusTotal与Google Safe Browsing对作者域名和第三方服务域名做恶意检测。

**📝 评价**
本文在规模和覆盖面上具有显著优势——786,036个应用跨六大平台历时五个月，是迄今LLM应用商店安全领域规模最大的实测研究，发现的15,146条误导性描述、1,366个隐私违规应用和15,996条有害内容具有较强现实意义。但存在若干明显局限：一是数据覆盖不均，GPT Store有663,119个应用却只逆向工程了22,961条指令（3.46%），严重影响第一、二层检测的代表性；Character.AI仅以ToxicDict关键词搜索采样，存在严重的选择偏差。二是检测方法缺乏严谨的ground truth验证——一致性分析的0.6阈值选取依据未充分论证，LLM检测器的精确率/召回率未在独立标注集上系统评估，文中仅列举少量正例而无混淆矩阵。三是"自精炼"机制极为有限，每轮仅随机抽取10条样例人工标注后更新，难以称得上真正的持续学习。四是ToxicDict的GPT-4o自动扩展和翻译环节可能引入LLM本身的偏差与幻觉，词典质量缺乏独立验证。整体而言论文更接近大规模安全审计报告，方法论的严密性和可复现性仍有提升空间。

---

### 82. Codebreaker: Dynamic Extraction Attacks on Code Language Models
